...
const {
  ...
  GOVERNANCE_SIGNER_KMS_KEY, RENEWER_SIGNER_KMS_KEY,
  CAPSULE_BUCKET, candidateWifPrincipal,
} = recon;
...
const resources = [
  ...
  ...[GOVERNANCE_SIGNER_KMS_KEY, RENEWER_SIGNER_KMS_KEY].map((k) => ({
  ...
  {
    label: `bucket ${CAPSULE_BUCKET}`,
    get: ["storage", "buckets", "get-iam-policy", `gs://${CAPSULE_BUCKET}`],
    set: (f) => ["storage", "buckets", "set-iam-policy", `gs://${CAPSULE_BUCKET}`, f],
  },
  {
