...
resource "google_project_iam_custom_role" "successor_bucket_iam" {
  ...
  description = "Can edit IAM policy only on bound auth-broker buckets for candidate window grants"
  permissions = [
...
resource "google_storage_bucket_iam_member" "activator_capsules_bucket_iam_admin" {
  bucket = google_storage_bucket.governance_state_capsules.name
  role   = google_project_iam_custom_role.successor_bucket_iam.id
  member = "serviceAccount:${var.successor_activation_service_account_email}"
}
