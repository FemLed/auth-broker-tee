...
import { recordAttestationRefresh } from "./governance-monitor.js";
  ...
  if (nonces.length === 0) {
    if (!cachedToken) {
      try {
        ...
        recordAttestationRefresh({ status: "success" });
      } catch (err) {
...
export function requestAttestationToken(nonces, { audience = ATTESTATION_AUDIENCE } = {}) {
